...
Varsayilan konum:
- B.A.R.B.A.R.O.S_WEATHER_LOCATION env varsa onu kullanir
...
def get_weather_summary(location: str | None = None) -> str:
    target = (location or os.environ.get("B.A.R.B.A.R.O.S_WEATHER_LOCATION") or "Istanbul").strip()
    try:
        response = requests.get(
            f"https://wttr.in/{target}",
            params={"format": "j1"},
            timeout=10,
            headers={"User-Agent": "B.A.R.B.A.R.O.S macOS"},
        )
